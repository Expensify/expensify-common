...
test("Test bold HTML replacement", () => {
  ...
  expect(parser.htmlToMarkdown(boldTestStartString)).toBe(
    boldTestReplacedString,
  );
});

test("Test multi-line bold HTML replacement", () => {
  const testString =
    "<strong>Here is a multi-line<br />comment that should<br />be bold</strong>";
...
test("Converts <b> tags with font-weight 700 inline style in between to markdown bold", () => {
  const input =
    '<b><span style="font-weight:400;">This is a text with </span><span style="font-weight:700;">nested bold</span><span style="font-weight:400;"> content</span></b>';
...
test("Does not convert <b> tags with font-weight normal inline style to markdown bold", () => {
  const input =
    '<b><span style="font-weight:400;">This is a text with </span><span style="font-weight:normal;">no bold</span><span style="font-weight:400;"> content</span></b>';
...
test("Test italic HTML replacement", () => {
  ...
  const italicTestReplacedString =
    ...
    "This is a _sentence,_ and it has some _punctuation, words, and spaces_. _test_ _ testing_ test_test_test. _ test _ _test _";
  expect(parser.htmlToMarkdown(italicTestStartString)).toBe(
    italicTestReplacedString,
  );
});

test("Test multi-line italic HTML replacement", () => {
  const testString =
    "<em>Here is a multi-line<br />comment that should<br />be italic</em>";
  const replacedString =
    "_Here is a multi-line\ncomment that should\nbe italic_";
...
test("Test strikethrough HTML replacement", () => {
  ...
  const strikethroughTestReplacedString =
    "This is a ~sentence,~ and it has some ~punctuation, words, and spaces~. ~test~ ~ testing~ test~test~test. ~ testing ~ ~testing ~";
  expect(parser.htmlToMarkdown(strikethroughTestStartString)).toBe(
    strikethroughTestReplacedString,
  );
});

test("Test multi-line strikethrough HTML replacement", () => {
  const testString =
    "<del>Here is a multi-line<br />comment that should<br />have strikethrough applied</del>";
  const replacedString =
...
test("Test Mixed HTML strings", () => {
  const rawHTMLTestStartString =
    "<em>This is</em> a <strong>test</strong>. None of <h2>these strings</h2> should display <del>as</del> <div>HTML</div>.";
  const rawHTMLTestReplacedString =
    "_This is_ a *test*. None of \nthese strings\n should display ~as~ \nHTML\n.";
  expect(parser.htmlToMarkdown(rawHTMLTestStartString)).toBe(
    rawHTMLTestReplacedString,
  );
});

test("Test HTML string with <br/> tags to markdown ", () => {
  const testString =
    "Hello<br/>World,<br/>Welcome<br/>To<br/>\nExpensify<br/>\n\nTwo new lines preceded by br";
  const resultString =
    "Hello\nWorld,\nWelcome\nTo\nExpensify\n\nTwo new lines preceded by br";
...
test("Test HTML string with attributes", () => {
  const testString =
    '<em style="color:red;">This is</em><br style="border-color:red;"> a <button disabled>test</button>. None of <strong data-link=\'bad\'>these strings</strong>.';
...
test("Test wrapped anchor tags", () => {
  ...
  expect(parser.htmlToMarkdown(wrappedUrlTestStartString)).toBe(
    wrappedUrlTestReplacedString,
  );
});
...
test("Test anchor tags where links end in a comma", () => {
  const testString =
    '<a href="https://github.com/Expensify/Expensify/issues/143231" target="_blank">https://github.com/Expensify/Expensify/issues/143231</a>,';
  const resultString =
    "[https://github.com/Expensify/Expensify/issues/143231](https://github.com/Expensify/Expensify/issues/143231),";
  expect(parser.htmlToMarkdown(testString)).toBe(resultString);
});

test("Test anchor tags where links have a period at the end", () => {
  const testString =
    '<a href="https://github.com/Expensify/ReactNativeChat/pull/645" target="_blank">https://github.com/Expensify/ReactNativeChat/pull/645</a>.';
  const resultString =
    "[https://github.com/Expensify/ReactNativeChat/pull/645](https://github.com/Expensify/ReactNativeChat/pull/645).";
  expect(parser.htmlToMarkdown(testString)).toBe(resultString);
});
...
test("Test HTML string with code fence", () => {
  const testString =
    '<pre id="code1">class Expensify extends PureComponent {\n    constructor(props) {\n        super(props);\n    }\n}</pre>';
  const resultString =
    "```\nclass Expensify extends PureComponent {\n    constructor(props) {\n        super(props);\n    }\n}\n```";
  expect(parser.htmlToMarkdown(testString)).toBe(resultString);

  const testStringWithBrTag =
    '<pre id="code1">class Expensify extends PureComponent {<br />    constructor(props) {<br />        super(props);<br />    }<br />}</pre>';
  ...
  const resultStringWithNewLinesFromSlack =
    "```\nline1\n\nline3\n\n\nline6\n```";
  expect(parser.htmlToMarkdown(testStringWithNewLinesFromSlack)).toBe(
    resultStringWithNewLinesFromSlack,
  );
});
...
test("Test code fence and extra backticks", () => {
  ...
  nestedBackticks =
    "<pre>&#x60;This&#32;is&#32;how&#32;you&#32;can&#32;write&#32;~strikethrough~,&#32;*bold*,&#32;_italics_,&#32;and&#32;[links](https://www.expensify.com)&#x60;</pre>";
  expect(parser.htmlToMarkdown(nestedBackticks)).toBe(
...
test("map div with bold and italics", () => {
  const testString =
    "<div><strong>line 1</strong><div></div><em>line 2</em></div>";
  const resultString = "*line 1*\n_line 2_";
  expect(parser.htmlToMarkdown(testString)).toBe(resultString);
});
...
test("map div with mixed html strings", () => {
  const testString =
    "<div><em>This is</em> a <strong>test</strong>. None of <h2>these strings</h2> should display <del>as</del><div>HTML</div><div></div><em>line 3</em></div>";
  const resultString =
    "_This is_ a *test*. None of \nthese strings\n should display ~as~\nHTML\n_line 3_";
  expect(parser.htmlToMarkdown(testString)).toBe(resultString);
});
...
test("triple quotes in same line", () => {
  const testString =
    "<blockquote><blockquote><blockquote>line 1</blockquote></blockquote></blockquote>";
  const resultString = ">>> line 1";
  expect(parser.htmlToMarkdown(testString)).toBe(resultString);
});
...
test("Test heading1 markdown replacement", () => {
  ...
  const resultString =
    "<h1>This is a heading1 because starts with # followed by a space</h1>";
  expect(parser.replace(testString)).toBe(resultString);
});
...
test("Test heading1 markdown replacement when # is followed by multiple spaces", () => {
  ...
  const resultString =
    "<h1>This is also a heading1 because starts with # followed by a space</h1>";
  expect(parser.replace(testString)).toBe(resultString);
});

test("Test heading1 markdown when # is not followed by a space", () => {
  const testString =
    "#This is not a heading1 because starts with a # but has no space after it\n";
  const resultString =
    "<mention-report>#This</mention-report> is not a heading1 because starts with a # but has no space after it<br />";
  expect(parser.replace(testString)).toBe(resultString);
});
...
test("Test html to heading1 markdown when h1 tag is in the end of the line", () => {
  const testString =
    "this line has an h1 in the end of the line<h1>heading1</h1>";
...
test("Test html to heading1 markdown when there are 2 h1 tags in the line", () => {
  const testString =
    "this is the first heading1<h1>heading1</h1>this is the second heading1<h1>heading1</h1>";
  const resultString =
    "this is the first heading1" +
    "\n# heading1" +
...
test("Map html paragraph to newline", () => {
  const testString =
    "<p>This is a HTML paragraph</p><p>This is an another HTML paragraph</p>";
  const resultString =
...
test("Map html paragraph with <br/> to newline", () => {
  const testString =
    "<p>This is a HTML paragraph</p><br/><p>This is an another HTML paragraph</p>";
  const resultString =
    "This is a HTML paragraph\n\nThis is an another HTML paragraph";
  expect(parser.htmlToMarkdown(testString)).toBe(resultString);
});
...
test("Map html list item to newline with two prefix spaces", () => {
  const testString =
    "<ul><li>This is a HTML list item</li><li>This is an another HTML list item</li></ul>";
  const resultString =
    "  This is a HTML list item\n  This is an another HTML list item";
  expect(parser.htmlToMarkdown(testString)).toBe(resultString);
});
...
test("Test list item replacement when there is an anchor tag inside <li> tag", () => {
  ...
  let resultString =
    "  [Coffee](https://example.com) -- Coffee\n  [Tea](https://example.com) -- Tea\n  [Milk](https://example.com) -- Milk";
  expect(parser.htmlToMarkdown(testString)).toBe(resultString);

  testString =
    '<ul><li><a href="http://localhost">Coffee</a> -- Coffee</li><li><a href="http://localhost/">Tea</a> -- Tea</li><li><a href="http://localhost/">Milk</a> -- Milk</li></ul>';
  resultString =
    "  [Coffee](http://localhost) -- Coffee\n  [Tea](http://localhost/) -- Tea\n  [Milk](http://localhost/) -- Milk";
  expect(parser.htmlToMarkdown(testString)).toBe(resultString);
});
...
test("Test blockquote linebreak handling with text, block and inline elements", () => {
  ...
  expect(parser.htmlToMarkdown(testStringSurroundedByText)).toBe(
    "text a\n> quote a\n\n> quote b\ntext b\n\ntext c\n> quote c\ntext c",
  );
  ...
  expect(parser.htmlToMarkdown(testStringSurroundedByInlineElement)).toBe(
    "_italic a_\n> quote a\n\n> quote b\n*bold b*\n\n_italic c_\n> quote c\n*bold c*",
  );
  ...
  expect(
    parser.htmlToMarkdown(testStringSurroundedByBlockElementCodeFence),
  ).toBe(
    "```\ncode fence a\n```\n> quote a\n\n> quote b\n```\ncode fence b\n```\n\n```\ncode fence c\n```\n> quote c\n```\ncode fence c\n```",
  );
  ...
  expect(parser.htmlToMarkdown(testStringSurroundedByBlockElementHeading)).toBe(
    "# h1 a\n> quote a\n\n> quote b\n# h1 b\n\n# h1 c\n> quote c\n# h1 c",
  );
});
...
test("Test codeFence copy from selection does not add extra new line", () => {
  let testString =
    "<div><div><comment><pre><div><span>code</span><br></div></pre>text</comment></div></div>";
  expect(parser.htmlToMarkdown(testString)).toBe("```\ncode\n```\ntext");
...
test("Linebreak should be remained for text between code block", () => {
  ...
  for (const { testString, resultString } of testCases) {
    expect(parser.htmlToMarkdown(testString)).toBe(resultString);
  }
});
...
describe("Image tag conversion to markdown", () => {
  ...
  test("Image with alt text containing escaped markdown", () => {
    const testString =
      '<img src="https://example.com/image.png" alt="&ast;bold&ast; &lowbar;italic&lowbar; &#126;strike&#126;" />';
    const resultString =
      "![*bold* _italic_ ~strike~](https://example.com/image.png)";
    expect(parser.htmlToMarkdown(testString)).toBe(resultString);
  });

  test("Image with alt text containing unescaped markdown", () => {
    const testString =
      '<img src="https://example.com/image.png" alt="*bold* _italic_ ~strike~" />';
    const resultString =
      "![*bold* _italic_ ~strike~](https://example.com/image.png)";
    expect(parser.htmlToMarkdown(testString)).toBe(resultString);
  });
  ...
  test("Cache extra attributes for img with alt with mediaAttributeCachingFn", () => {
    ...
    const testString =
      '<img src="https://example.com/image.png" alt="altText" data-expensify-width="100" data-expensify-height="500" data-name="newName" data-expensify-source="expensify-source" />';
    ...
    expect(mediaAttributeCachingFn).toHaveBeenCalledWith(
      "https://example.com/image.png",
      'data-expensify-width="100" data-expensify-height="500" data-name="newName" data-expensify-source="expensify-source"',
    );
  });
  ...
  test("Cache extra attributes for img with alt with cacheVideoAttributes", () => {
    ...
    expect(parser.htmlToMarkdown(testString, extras)).toBe(resultString);
    expect(cacheVideoAttributes).toHaveBeenCalledWith(
      "https://example.com/image.png",
      'data-expensify-width="100" data-expensify-height="500" data-name="newName" data-expensify-source="expensify-source"',
    );
  });
  ...
  test("Cache extra attributes for img without alt with mediaAttributeCachingFn", () => {
    const mediaAttributeCachingFn = jest.fn();
    const testString =
      '<img src="https://example.com/image.png" data-expensify-width="100" data-expensify-height="500" data-name="newName" data-expensify-source="expensify-source" />';
    const resultString = "!(https://example.com/image.png)";
    const extras = {
      mediaAttributeCachingFn,
    };
...
describe("Video tag conversion to markdown", () => {
  test("Video with name", () => {
    const testString =
      '<video data-expensify-source="https://example.com/video.mp4">video</video>';
  ...
  test("Video without name", () => {
    const testString =
      '<video data-expensify-source="https://example.com/video.mp4"></video>';
  ...
  test("Video with extra attributes to be cached with cacheVideoAttributes", () => {
    const cacheVideoAttributes = jest.fn();
    const testString =
      '<video data-expensify-source="https://example.com/video.mp4" data-expensify-width="100" data-expensify-height="500" data-expensify-thumbnail-url="https://image.com/img.jpg">video</video>';
    const resultString = "![video](https://example.com/video.mp4)";
    const extras = {
      cacheVideoAttributes,
    };
    ...
    expect(cacheVideoAttributes).toHaveBeenCalledWith(
      ...
      ' data-expensify-width="100" data-expensify-height="500" data-expensify-thumbnail-url="https://image.com/img.jpg"',
    );
  });
