...
export default class ExpensiMark {
        ...
        this.rules = [
             ...
             */
            {
                ...
                replacement: (match, g1, g2, g3) => {
                    ...
                    return `<a href="${g3 && g3.includes('http') ? '' : 'http://'}${g2}" target="_blank" rel="noreferrer noopener">${g1}</a>`;
                },
            },
             ...
             */
            {
                ...
                // If there is not, we force the link to be absolute by prepending '//' to the target.
                replacement: (match, g1, g2, g3) => (
                    `${g1}<a href="${g3 && g3.includes('http') ? '' : 'http://'}${g2}" target="_blank" rel="noreferrer noopener">${g2}</a>${g1}`
                ),
            },
            {
                /**
     ...
     * const text = 'This is a mark down styled text, *bold*, _italic_, ~strikethrough~';
     * const options = {
     *   filterRules: ['bold', 'italic'],
     *   isTextEscaped: true,
     * };
     *
     * // It only applies bold and italic rules
     * parser.replace(text, options);
     *
     ...
     * @param {Boolean} [options.isTextEscaped=false] - Whether the text is already escaped
     *
     * @returns {String}
     */
    replace(text, options) {
        const filterRules = lodashGet(options, 'filterRules', []);
        const isTextEscaped = lodashGet(options, 'isTextEscaped', false);

        // This ensures that any html the user puts into the comment field shows as raw html
        let replacedText = isTextEscaped ? text : Str.safeEscape(text);
    ...
    modifyTextForUrlLinks(regex, textToCheck, replacement) {
        ...
        while (match !== null) {
            ...
            if (abort) {
                ...
                // match[1] contains the text inside [] and we need to parse this text.
                // This can also contain formatted text but we are restricting to only bold, italic
                // and strikethrough because we do not want a link inside this text too.
                // Note: As the rules are applied in order as defined. This text is already formatted
                // with the rules that preceeds link rule (eg, codeFence, inlineCodeBlock, email)
                // and is already escaped.
