...
export default class ExpensiMark {
    constructor() {
        ...
        this.rules = [
             ...
             */
            {
                ...
                process: (regex, textToProcess, replacement) => {
                    return this.modifyTextForLinks(regex, textToProcess, replacement);
                },
            ...
            },
            {
                ...
                // regex: new RegExp(`(?![^<]*>|[^<>]*<\\/)([_*~]*?)(https?:\\/\\/)?[\\w\\-~]+(\\.[\\w\\-~]+)+(\\/[\\w\\-~@:%]*)*(#[\\w\\-]*)?(\\?[^\\s]*)?\\1(?![^<]*(<\\/pre>|<\\/code>))`, 'gi'),
    ...
    replace(text) {
        ...
        this.rules.forEach((rule) => {
            ...
            if (rule.process) {
                replacedText = rule.process(rule.regex, replacedText, rule.replacement);
            } else {
     ...
     * @returns {String}
     */
    modifyTextForLinks(regex, textToCheck, replacement) {
        var match;
        var replacedText = '';
        var	startIndex = 0;
        var	abort = false;
        ...
        while ((match = regex.exec(textToCheck)) != null) {
            ...
            // we want to avoid matching punctuation at the end of links (textToCheck[match.index + match[0].length - 1] === '?')
            ...
            if (abort) {
                ...
                //replacedText = replacedText.concat(`${match[1]}<a href="${match[3] && match[3].startsWith('http') ? '' : 'http://'}${match[2]}" target="_blank">${match[2]}</a>${match[1]}`);
            }
