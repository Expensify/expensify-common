    ...
    constructor() {
        ...
        this.rules = [
             ...
             */
            {
                ...
                replacement: (match, g1, g2, ...args) => {
                    // g1 will be video file name, g2 will be video source (video URL)
                    const extras = args[args.length - 1];
                    const extraAttrs = extras && extras.videoAttributeCache && extras.videoAttributeCache[g2];
                    return `<video data-expensify-source="${Str.sanitizeURL(g2)}" ${extraAttrs || ''}>${g1 ? `${g1}` : ''}</video>`;
                },
                rawInputReplacement: (match, g1, g2, ...args) => {
                    const extras = args[args.length - 1];
                    const extraAttrs = extras && extras.videoAttributeCache && extras.videoAttributeCache[g2];
                    return `<video data-expensify-source="${Str.sanitizeURL(g2)}" data-raw-href="${g2}" data-link-variant="${typeof g1 === 'string' ? 'labeled' : 'auto'}" ${extraAttrs || ''}>${g1 ? `${g1}` : ''}</video>`;
                },
            },
        ...
        this.htmlToMarkdownRules = [
...

            {
                 ...
                 * @param {string} g2 {string} or group2 will be the video source (video URL)
                 * @param {string} g3 will be the video attributes (data-expensify-width, data-expensify-height, etc...)
                 * @param {string} g4 or group4 will be the video file name (the text between opening and closing video tags)
                 ...
                 */
                replacement: (match, g1, g2, g3, g4, ...args) => {
                    const extras = args[args.length - 1];
                    if (g3 && extras && extras.cacheVideoAttributes && typeof extras.cacheVideoAttributes === 'function') {
                        extras.cacheVideoAttributes(g2, g3);
                    }
                    if (g4) {
                        return `![${g4}](${g2})`;
                    }
