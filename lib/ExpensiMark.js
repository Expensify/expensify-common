...
export default class ExpensiMark {
    ...
    truncateMarkdown(text, limit, ellipsis) {
        ...
        const html = this.replace(text);
    
        // Remove HTML tags to get plain text
        const plainText = this.htmlToText(html);
    
        // Check if the plain text exceeds the limit
        if (plainText.length <= limit) {
            return text;
        }
    
        // Find the last word within the limit
        const truncatedText = plainText.slice(0, limit);
        let lastSpaceIndex = truncatedText.lastIndexOf(' ');
    
        // If the last space is not found or is too close to the limit,
        // find the next space after the limit to avoid cutting off words
        if (lastSpaceIndex === -1 || limit - lastSpaceIndex <= 10) {
            const nextSpaceIndex = plainText.indexOf(' ', limit);
            if (nextSpaceIndex !== -1) {
                lastSpaceIndex = nextSpaceIndex;
            }
        }
    
        const truncatedPlainText = plainText.slice(0, lastSpaceIndex);
    
        // Find the corresponding position in the original HTML
        const truncatedHtmlEndIndex = html.indexOf(truncatedPlainText) + truncatedPlainText.length;
        let truncatedHtml = html.slice(0, truncatedHtmlEndIndex);
    
        // Find the last word in the truncated HTML between tags
        const lastWordRegex = />([^<]+)$/;
        const lastWordMatch = truncatedHtml.match(lastWordRegex);
        if (lastWordMatch) {
            const lastWord = lastWordMatch[1].trim();
            const lastWordIndex = truncatedHtml.lastIndexOf(lastWord);
            truncatedHtml = truncatedHtml.slice(0, lastWordIndex + lastWord.length);
        }
    
        // Find the nearest closing tag after the truncated position
        const closingTagRegex = /<\/[^>]+>/g;
        const closingTagMatch = closingTagRegex.exec(html.slice(truncatedHtml.length));
        if (closingTagMatch) {
            truncatedHtml += closingTagMatch[0];
        }
    
        // Convert the truncated HTML back to Markdown
        let truncatedMarkdown = this.htmlToMarkdown(truncatedHtml);
    
        // Add ellipsis if specified
        if (ellipsis) {
            truncatedMarkdown = `${truncatedMarkdown.trim()  }...`;
        }
