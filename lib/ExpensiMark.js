...
export default class ExpensiMark {
    ...
    modifyTextForLinks(regex, textToCheck, replacement) {
        ...
        while ((match = regex.exec(textToCheck)) != null) {
            ...
            // we want to avoid matching punctuation at the end of links
            if (endingPunctuation.includes(textToCheck[match.index + match[0].length - 1])) {
                match[0] = match[0].substr(0, match[0].length - 1);
                match[2] = match[2].substr(0, match[2].length - 1);
            }

            // we want to avoid matching ending ) unless it is a closing parenthesis for params
            if (textToCheck[match.index + match[0].length - 1] === ')' && !match[0].includes('(')) {
                match[0] = match[0].substr(0, match[0].length - 1);
                match[2] = match[2].substr(0, match[2].length - 1);
            }
