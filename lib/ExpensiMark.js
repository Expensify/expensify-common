...
export default class ExpensiMark {
    ...
    replace(text, rules = []) {
        ...
        const currentRules = _.isEmpty(rules) ? this.rules : _.filter(this.rules, rule => _.contains(rules, rule.name));
    ...
    modifyTextForUrlLinks(regex, textToCheck, replacement) {
        ...
        while (match !== null) {
            ...
            if (abort) {
                ...
                // we are allowing only bold, strikethrough and italic formatting for link text
                const linkText = this.replace(match[1], ['bold', 'strikethrough', 'italic']);
