    ...
    constructor() {
        ...
        this.rules = [
             ...
             */
            {
                ...
                replacement: (match, g1, g2) => `<img src="${Str.sanitizeURL(g2)}"${g1 ? ` alt="${g1}"` : ''} />`,
                rawInputReplacement: (match, g1, g2) => `<img src="${Str.sanitizeURL(g2)}"${g1 ? ` alt="${g1}"` : ''} data-raw-href="${g2}" data-link-variant="labeled" />`
            },
            ...
            {
                // This rule is designed to address cases where HTML content ends up within attribute values,
                // HTML can end up within attributes, because different rules are applied in sequence
                // To ensure cleanliness and correctness of HTML attributes, this cleanup rule is executed last.
                // We're limiting the scope of this rule to 'img' tags and their 'alt' attributes, which are the
                // primary sources of such issues. This approach keeps the operation efficient and focused.
                // However, the pattern is extensible to additional tags and attributes as needed.
                name: 'attributeCleanup',
                regex: /<(?:img)[^>]*\s(?:alt)=(["'])([^"']*)\1[^>]*>/gi,
                replacement: (match, g1, g2) => {
                    const cleanedG2 = this.htmlToMarkdown(g2);
                    if (cleanedG2 === g2) {
                        return match;
                    }

                    return match.replace(g2, _.escape(cleanedG2));
                }
            }
        ];
