...
export default class ExpensiMark {
    ...
    truncate(text, limit, ellipsis) {
        ...
        const truncateString = (tText) => {
            const formatStack = [];
            let skipCountIncrement = false;
            let outputText = "";
            let index = 0;

            while (count < limit && index < tText.length) {
                const formatMarker = findFormatMarkerAhead(
                    tText.substring(index),
                    formatStack
                );
                if (formatMarker) {
                ...
                const formatPlaceholder = findFormatPlaceholderAhead(
                    tText.substring(index)
                );
                ...
                const hyperlinkMatch = hyperlinkAheadRegexp.exec(
                    tText.substring(index)
                );
                if (hyperlinkMatch) {
