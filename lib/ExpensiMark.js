...
export default class ExpensiMark {
    ...
    findFormatPlaceholderAhead(text) {
        const formatPlaceholders = _.keys(formatPlaceholdersMap);
    ...
    replaceFormatMarkersWithPlaceholders(text) {
        return text
            ...
            .replace(UNDERSCORE_BOLD_REGEXP, `${UNDERSCORE_BOLD_PLACEHOLDER_REGEXP.source}$2${UNDERSCORE_BOLD_PLACEHOLDER_REGEXP.source}`)
            .replace(ASTERISK_BOLD_REGEXP, `${ASTERISK_BOLD_PLACEHOLDER_REGEXP.source}$2${ASTERISK_BOLD_PLACEHOLDER_REGEXP.source}`)
            .replace(UNDERSCORE_ITALIC_REGEXP, `${UNDERSCORE_ITALIC_PLACEHOLDER_REGEXP.source}$2${UNDERSCORE_ITALIC_PLACEHOLDER_REGEXP.source}`)
            .replace(ASTERISK_ITALIC_REGEXP, `${ASTERISK_ITALIC_PLACEHOLDER_REGEXP.source}$2${ASTERISK_ITALIC_PLACEHOLDER_REGEXP.source}`);
    }
    ...
    truncateMarkdown(text, limit, ellipsis) {
        ...
        const truncateString = (tempText) => {
            ...
            for (let index = 0; count < limit && index < tempText.length; index++) {
                ...
                const formatPlaceholder = this.findFormatPlaceholderAhead(substring);
