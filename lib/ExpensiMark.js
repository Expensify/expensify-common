...
export default class ExpensiMark {
        ...
        this.rules = [
             ...
             */
            {
                ...
                replacement: (match, g1, g2, g3) => (
                    `<a href="${g3 && g3.includes('http') ? '' : 'http://'}${g2}" target="_blank" rel="noreferrer noopener">${g1}</a>`
                ),
            },
    ...
    mapBlock(htmlString) {
        ...
        while (splitText.length) {
            if (splitText[splitText.length - 1].trim() === 0 && !splitText[splitText.length - 1].match(/\n/)) {
                splitText.pop();
            } else {
                break;
            }
        }
        for (let index = 0; index < splitText.length - 1; index++) {
            if (splitText[index].trim() || splitText[index].match(/\n/)) {
                if (splitText[index].match(/[\n|>]$/)) {
                    joinedText += splitText[index];
                } else {
                    joinedText += `${splitText[index]}\n`;
                }
            }
        ...
        // last div will not map to new line
        joinedText += splitText[splitText.length - 1];
        return joinedText;
    }
    ...
    htmlToMarkdown(htmlString) {
        ...
        generatedMarkdown = generatedMarkdown.replace(/<div.*?>|<\/div>|<comment.*?>|<\/comment>|<h1>|<\/h1>|<h2>|<\/h2>|<h3>|<\/h3>|<h4>|<\/h4>|<h5>|<\/h5>|<h6>|<\/h6>/gm, "[block]")
        return Str.htmlDecode(this.mapBlock(Str.stripHTML(generatedMarkdown)));
    }
