...
const SLACK_SPAN_NEW_LINE_TAG =
    '<span class="c-mrkdwn__br" data-stringify-type="paragraph-break" style="box-sizing: inherit; display: block; height: unset;"></span>';
...
export default class ExpensiMark {
    constructor() {
        ...
        this.rules = [
             ...
             */
            {
                ...
                rawInputReplacement: (match, __, textWithinFences) => {
                    const withinFences = match.replace(
                        /(?:&#x60;&#x60;&#x60;)([\s\S]*?)(?:&#x60;&#x60;&#x60;)/g,
                        "$1"
                    );
                    const group = textWithinFences.replace(
             ...
             */
            {
                ...
                process: (textToProcess, replacement, shouldKeepRawInput) => {
                    ...
                    return this.modifyTextForEmailLinks(
                        regex,
                        textToProcess,
                        replacement,
                        shouldKeepRawInput
             ...
             */
            {
                ...
                replacement: (match, g1, g2) =>
                    `<img src="${Str.sanitizeURL(
                        g2
                    )}" alt="${this.escapeMarkdownEntities(g1)}" />`,
                rawInputReplacement: (match, g1, g2) =>
                    `<img src="${Str.sanitizeURL(
                        g2
                    )}" alt="${this.escapeMarkdownEntities(
                        g1
                    )}" data-raw-href="${g2}" data-link-variant="labeled" />`,
            },
             ...
             */
            {
                ...
                replacement: (match, g1, g2) => {
                    ...
                    return `<a href="${Str.sanitizeURL(
                        g2
                    )}" target="_blank" rel="noreferrer noopener">${g1.trim()}</a>`;
                },
                rawInputReplacement: (match, g1, g2) => {
                    if (g1.match(CONST.REG_EXP.EMOJIS) || !g1.trim()) {
                        return match;
                    ...
                    return `<a href="${Str.sanitizeURL(
                        g2
                    )}" data-raw-href="${g2}" data-link-variant="labeled" target="_blank" rel="noreferrer noopener">${g1.trim()}</a>`;
                },
            },
             ...
             */
            {
                name: "userMentions",
                regex: new RegExp(
                    `(@here|[a-zA-Z0-9.!$%&+=?^\`{|}-]?)(@${CONST.REG_EXP.EMAIL_PART}|@${CONST.REG_EXP.PHONE_PART})(?!((?:(?!<a).)+)?<\\/a>|[^<]*(<\\/pre>|<\\/code>))`,
                    "gim"
                ),
                replacement: (match, g1, g2) => {
                    if (!Str.isValidMention(match)) {
                    ...
                    const phoneRegex = new RegExp(
                        `^@${CONST.REG_EXP.PHONE_PART}$`
                    );
                    return `${g1}<mention-user>${g2}${
                        phoneRegex.test(g2) ? `@${CONST.SMS.DOMAIN}` : ""
                    }</mention-user>`;
                },
            },
...

            {
                ...
                process: (
                    ...
                    if (shouldKeepRawInput) {
                        return textToProcess.replace(regex, (g1) =>
                            replacement(g1, shouldKeepRawInput)
                        );
                    }
                    return this.modifyTextForQuote(
                        regex,
                        textToProcess,
                        replacement
                    );
                ...
                replacement: (g1, shouldKeepRawInput = false) => {
                    ...
                    const textToReplace = g1.replace(
                        /^&gt;( )?/gm,
                        (match, g2) => {
                            if (shouldKeepRawInput) {
            ...
            },
            {
                ...
                replacement: (
                    match,
                    extraLeadingUnderscores,
                    textWithinUnderscores
                ) => {
                    if (
                        textWithinUnderscores.includes("</pre>") ||
                        this.containsNonPairTag(textWithinUnderscores)
                    ) {
                        return match;
                    }
                    if (
                        String(textWithinUnderscores).match(
                            `^${CONST.REG_EXP.MARKDOWN_EMAIL}`
                        )
                    ) {
             ...
             */
            {
                ...
                rawInputReplacement:
                    '$1<a href="mailto:$2" data-raw-href="$2" data-link-variant="auto">$2</a>',
            },

            {
                ...
                replacement: (match, g1) =>
                    g1.includes("</pre>") || this.containsNonPairTag(g1)
                        ? match
                        : `<strong>${g1}</strong>`,
            },
            {
        ...
        this.htmlToMarkdownRules = [
            ...
            },
            {
                ...
                pre: (inputString) =>
                    inputString
                        ...
                        .replace("<br/></tbody>", "")
                        .replace(
                            SLACK_SPAN_NEW_LINE_TAG + SLACK_SPAN_NEW_LINE_TAG,
                            "<br/><br/><br/>"
                        )
            ...
            },
            {
                ...
                replacement: (match, g1, g2) => {
                    ...
                    const resultString = g2
                        .replace(/\n?(<h1># )/g, "$1")
                        .replace(/(<h1>|<\/h1>)+/g, "\n")
                        .trim()
                        .split("\n")
                        .map((m) => `> ${m.replace(/<\/?blockquote>/g, "> ")}`)
                        .join("\n");
         ...
         * @type {Object[]}
         */
        this.shouldKeepWhitespaceRules = this.rules.filter(
            (rule) => !this.whitespaceRulesToDisable.includes(rule.name)
        );
    ...
    replace(
        ...
        const enabledRules = shouldKeepRawInput
            ? this.shouldKeepWhitespaceRules
            : this.rules;
        const rules = _.isEmpty(filterRules)
            ? enabledRules
            : _.filter(this.rules, (rule) =>
                  _.contains(filterRules, rule.name)
              );
        ...
        try {
            rules.forEach((rule) => {
                ...
                const replacementFunction =
                    shouldKeepRawInput && rule.rawInputReplacement
                        ? rule.rawInputReplacement
                        : rule.replacement;
                if (rule.process) {
                    replacedText = rule.process(
                        replacedText,
                        replacementFunction,
                        shouldKeepRawInput
                    );
            ...
            console.warn(
                "Error replacing text with html in ExpensiMark.replace",
                { error: e }
            );
    ...
    modifyTextForUrlLinks(regex, textToCheck, replacement) {
        ...
        while (match !== null) {
            ...
            replacedText = replacedText.concat(
                textToCheck.substr(startIndex, match.index - startIndex)
            );
            ...
            if (match.index !== 0 && textToCheck[match.index - 1] === "@") {
                ...
                if (domainMatch !== null && domainMatch[3] !== "") {
                    replacedText = replacedText.concat(
                        domainMatch[1] +
                            this.replace(domainMatch[3], {
                                filterRules: ["autolink"],
                            })
                    );
            ...
            if (
                isDoneMatching ||
                match[1].includes("</pre>") ||
                match[1].includes("</h1>")
            ) {
                replacedText = replacedText.concat(
                    textToCheck.substr(match.index, match[0].length)
                );
                ...
                replacedText = replacedText.concat(
                    replacement(match[0], linkText, url)
                );
            }
    ...
    modifyTextForEmailLinks(
        ...
        while (match !== null) {
            ...
            const replacedMatch = shouldKeepRawInput
                ? replacement(match[0], linkText, match[2], match[3])
                : replacement(match[0], linkText, match[3]);
    ...
    replaceBlockElementWithNewLine(htmlString) {
        ...
        while (splitText.length) {
            if (
                splitText[splitText.length - 1].trim().length > 0 ||
                splitText[splitText.length - 1].match(/\n/)
            ) {
                break;
            }
            splitText.pop();
    ...
    htmlToMarkdown(htmlString) {
        ...
        return Str.htmlDecode(
            this.replaceBlockElementWithNewLine(generatedMarkdown)
        );
    }
        ...
        if (match !== null) {
            ...
            for (let i = 0; i < textSplitted.length; i++) {
                ...
                if (
                    ...
                    if (textToFormat !== "") {
                        replacedText += this.formatTextForQuote(
                            regex,
                            textToFormat,
                            replacement
                        );
    ...
    formatTextForQuote(regex, textToCheck, replacement) {
        if (textToCheck.match(regex)) {
            ...
            let textToFormat = textToCheck
                .split("\n")
                .map((row) => row.substr(4).trim())
                .join("\n");
    ...
    extractLinksInMarkdownComment(comment) {
        try {
            ...
            const regex = new RegExp(
                `<a href="${MARKDOWN_URL_REGEX}" target="_blank" rel="noreferrer noopener">`,
                "gi"
            );
            ...
            // eslint-disable-next-line no-console
            console.warn(
                "Error parsing url in ExpensiMark.extractLinksInMarkdownComment",
                { error: e }
            );
            return undefined;
        }
    ...
    getRemovedMarkdownLinks(oldComment, newComment) {
        ...
        return linksInOld === undefined || linksInNew === undefined
            ? []
            : _.difference(linksInOld, linksInNew);
    }
    ...
    truncateMarkdown(text, limit, ellipsis) {
        ...
        const truncateString = (tempText) => {
            ...
            for (
                let index = 0;
                count < limit && index < tempText.length;
                index++
            ) {
                const substring = tempText.substring(index);
                const formatMarker = this.findFormatMarkerAhead(
                    substring,
                    formatStack
                );
                if (formatMarker) {
