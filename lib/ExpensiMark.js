...
export default class ExpensiMark {
    ...
    truncateHTML(html, maxLength, options) {
        ...
        function closeTags(tags) {
            return _.map(tags.reverse(), (mappedTag) => `</${mappedTag}>`).join('');
        }

        function getEndPosition(content, tailPos) {
            const defaultPos = maxLength - totalLength;
            const slop = opts.slop !== undefined ? opts.slop : DEFAULT_SLOP;
            let position = defaultPos;
            const isShort = defaultPos < slop;
            const slopPos = isShort ? defaultPos : slop - 1;
            const substr = content.slice(isShort ? 0 : defaultPos - slop, tailPos !== undefined ? tailPos : defaultPos + slop);
