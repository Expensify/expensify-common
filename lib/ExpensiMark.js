...
export default class ExpensiMark {
    constructor() {
        ...
        this.rules = [
            ...
            {
                name: 'link',
                regex: new RegExp(`\\[([\\w\\s\\d!?]+)\\]\\((((?:(?:https?):\\/\\/)?(?:www\\.)?)[a-z0-9]+\\.(?:${TLD_REGEX}))\\)($|\\s|[.,;~_*])(?![^<]*(<\\/pre>|<\\/code>))`, 'gi'),
                ...
                replacement: (match, g1, g2, g3, g4) => {
                    return `<a href="${g3.includes('http') ? '' : '//'}${g2}" target="_blank">${g1}</a>${g4}`
                },
            },
            {
                name: 'autolink',
                // eslint-disable-next-line max-len
                //regex: new RegExp(`(?![^<]*>|[^<>]*<\\/)([_*~]*?)(((?:https:\\/\\/)?(www\\.)?)(?:[a-z]+[^\\s])?[^\\s<>*~_"\'\´.-][^\\s<>"\'\´]*?\\.(?:${TLD_REGEX}))\\1\\b(?![^<]*(<\\/pre>|<\\/code>))`, 'gi'),
                regex: new RegExp(`(?![^<]*>|[^<>]*<\\/)([_*~]*?)(((?:https?:\\/\\/)?(www\\.)?)(?:[a-z]+[^\\s])?\\.(?:${TLD_REGEX})(?:\\/[^\\s]*)?\\b)\\1(?![^<]*(<\\/pre>|<\\/code>))`, 'gi'),
                // We use a function here to check if there is already a https:// on the link. If there is not, we force
                // the link to be absolute by prepending '//' to the target.
                replacement: (match, g1, g2, g3) => {
                    return `${g1}<a href="${g3.includes('http') ? '' : '//'}${g2}" target="_blank">${g2}</a>${g1}`
                },
            },
            {
                /**
            ...
            {
                name: 'newline',
                // We have this: ([^>]) because we don't want to be adding <br> tags between other html tags
                regex: /([^>])(\n)/g,
                replacement: '$1<br>',
            },
        ];
