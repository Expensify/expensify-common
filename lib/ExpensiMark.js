...
export default class ExpensiMark {
    constructor() {
        ...
        this.rules = [
             ...
             */
            {
                ...
                replacement: (match, g1, g2) => `<img src="${StrUtils.Str.sanitizeURL(g2)}" alt="${this.escapeMarkdownEntities(g1)}" />`,
                rawInputReplacement: (match, g1, g2) => `<img src="${StrUtils.Str.sanitizeURL(g2)}" alt="${this.escapeMarkdownEntities(g1)}" data-raw-href="${g2}" data-link-variant="labeled" />`
            },
             ...
             */
            {
                ...
                replacement: (match, g1, g2) => {
                    ...
                    return `<a href="${StrUtils.Str.sanitizeURL(g2)}" target="_blank" rel="noreferrer noopener">${g1.trim()}</a>`;
                },
                rawInputReplacement: (match, g1, g2) => {
                    if (g1.match(Constants.CONST.REG_EXP.EMOJIS) || !g1.trim()) {
                        return match;
                    }
                    return `<a href="${StrUtils.Str.sanitizeURL(g2)}" data-raw-href="${g2}" data-link-variant="labeled" target="_blank" rel="noreferrer noopener">${g1.trim()}</a>`;
                },
            },
             ...
             */
            {
                ...
                replacement: (match, g1, g2, g3) => {
                    if (!StrUtils.Str.isValidMention(match)) {
                        return match;
                    }
             ...
             */
            {
                ...
                replacement: (match, g1, g2) => {
                    const href = StrUtils.Str.sanitizeURL(g2);
    ...
    replaceBlockElementWithNewLine(htmlString) {
        ...
        splitText = _.map(splitText, (text) => StrUtils.Str.stripHTML(text));
        let joinedText = '';
    ...
    htmlToMarkdown(htmlString) {
        ...
        return StrUtils.Str.htmlDecode(this.replaceBlockElementWithNewLine(generatedMarkdown));
    }
    ...
    htmlToText(htmlString) {
        ...
        replacedText = StrUtils.Str.htmlDecode(replacedText);
        return replacedText;
    }
        ...
        if (match !== null) {
            ...
            for (let i = 0; i < textSplitted.length; i++) {
                if (!insideCodefence) {
                    ...
                    insideCodefence = StrUtils.Str.contains(textSplitted[i], '<pre>');
                }

                // We only want to modify lines starting with &gt; that is not codefence
                if (StrUtils.Str.startsWith(textSplitted[i], '&gt;') && !insideCodefence) {
    ...
    extractLinksInMarkdownComment(comment) {
        try {
            ...
            const links = _.map(matches, (match) => StrUtils.Str.sanitizeURL(match[1]));
            return links;
        } catch (e) {
            // eslint-disable-next-line no-console
