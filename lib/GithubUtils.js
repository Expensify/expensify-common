...
export default class GithubUtils {
     ...
     */
    getIssueOrPullRequestNumberFromURL(URL) {
        const matches = URL.match(/https:\/\/github\.com\/.*\/.*\/(?:issues|pull)\/([0-9]+).*/);
        if (!_.isArray(matches) || matches.length !== 2) {
            throw new Error(`Provided URL ${URL} is not a Github Issue or Pull Request!`);
        }
        return matches[1];
    }
    ...
    generateStagingDeployCashBody(
        ...
        const sortedDeployBlockers = _.sortBy(_.unique(deployBlockers), URL => this.getIssueOrPullRequestNumberFromURL(URL));
