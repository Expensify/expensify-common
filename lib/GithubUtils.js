...
const ISSUE_OR_PULL_REQUEST_REGEX = new RegExp('https://github.com/.*/.*/(?:pull|issues)/([0-9]+).*');
...
export default class GithubUtils {
    ...
    getStagingDeployCashPRList(issue) {
        ...
        const unverifiedPRs = _.map(
            [...PRListSection.matchAll(new RegExp(`- \\[ ] (${ISSUE_OR_PULL_REQUEST_REGEX.source})`, 'g'))],
            match => ({
                url: match[1],
        ...
        const verifiedPRs = _.map(
            [...PRListSection.matchAll(new RegExp(`- \\[x] (${ISSUE_OR_PULL_REQUEST_REGEX.source})`, 'g'))],
            match => ({
                url: match[1],
                number: GithubUtils.getPullRequestNumberFromURL(match[1]),
                isVerified: true,
    ...
    updateStagingDeployCash(newTag, newPRs, newDeployBlockers) {
        return this.getStagingDeployCash()
            .then(({
                ...
                const issueNumber = GithubUtils.getIssueNumberFromURL(url);
                ...
                return this.octokit.issues.update({
                    ...
                    body: this.generateStagingDeployCashBody(
                        newTag,
                        PRList,
                        _.filter(PRList, PR => PR.isVerified),
                        deployBlockers,
                        _.filter(deployBlockers, deployBlocker => deployBlocker.isResolved)
                        ),
                });
            });
    }
