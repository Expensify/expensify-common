...
class DropDown extends React.Component {
    ...
    renderOption(option) {
        return (
            <DropDownItem
                ...
                children={option.children || null}
                disabled={option.disabled || false}
                focused={option.focused || false}
                isSelected={option.isSelected || false}
                isSelectable={option.isSelectable || true}
                divider={option.divider || false}
                extraClasses={option.extraClasses || []}
    ...
    render() {
        ...
        return <ul className={cn('dropdown-menu', extraClasses)}>{options.map(this.renderOption)}</ul>;
    }
}
