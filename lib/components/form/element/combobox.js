...
module.exports = React.createClass({
    propTypes: {
        ...
        options: React.PropTypes.arrayOf(
            React.PropTypes.shape({
                ...
                hasError: React.PropTypes.boolean,
                // If this option is a header, then it can't be selected and needs styled differently
        ...
        // By default we show the selected value, you can overwrite it to show this
        showSelectedText: React.PropTypes.bool
    },
    ...
    getInitialState(noDefaultValue, newOptions) {
        ...
        const truncatedOptions = _(options).first(this.props.maxItemsToShow).map(option => ({
            text: option.text,
            value: option.value,
        ...
        } else if (options.length > this.props.maxItemsToShow) {
            truncatedOptions.push({
        ...
        const defaultSelectedOption = _(options).findWhere({value: this.props.defaultValue});
    ...
    setValue(val) {
        ...
        const newOptions = _(this.state.options).map((option) => {
    ...
    reset(noDefaultValue, newOptions) {
        this.setState(this.getInitialState(noDefaultValue, newOptions));
    },
    ...
    resetFocusedElements() {
        this.setState({
            options: _(this.state.options).map((option) => {
    ...
    resetClickAwayHandler() {
        $('body').off('click.clickaway', this.closeDropdown);
        if (this.state.isDropdownOpen) {
            $('body').on('click.clickaway', this.closeDropdown);
        } else {
    ...
    handleKeyDown(e) {
        ...
        case KeyboardShortcutManager.MAPPINGS.KEY_ENTER:
            ...
            if (this.state.options[this.state.focusedIndex].isHeader) {
                break;
            }
    ...
    performSearch(e) {
        ...
        for (let i = 0; i < matchRegexes.length; i++) {
            if (matches.size < this.props.maxSearchResults) {
                for (let j = 0; j < this.props.options.length; j++) {
                    // Don't include the header options
                    if (!this.props.options[j].isHeader && matchRegexes[i].test(this.props.options[j].text.toString().replace(new RegExp(/&nbsp;/g), ''))) {
                        matches.add(this.props.options[j]);
                    }
        ...
        const defaultSelectedOption = _(options).findWhere({value: e.target.value});
    ...
    handleClick(selectedValue) {
        ...
        let currentText = '';
        const newOptions = _(this.state.options).each((option) => {
        ...
        return (
            ...
            >
                <input
                    ...
                    value={this.props.showSelectedText ? this.state.currentText.replace(new RegExp(/&nbsp;/g), '') : this.state.currentValue}
