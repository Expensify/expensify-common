...
class Combobox extends React.Component {
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        ...
        if (!_.isUndefined(nextProps.options)) {
            ...
            if (has(nextProps.options, '0.id')) {
                if (
                    !_.isEqual(_.pluck(nextProps.options, 'id'), _.pluck(this.props.options, 'id')) ||
    ...
    getStartState(noDefaultValue, newOptions, newAlreadySelectedOptions) {
        ...
        const removeSMSDomainIfPhoneNumber = (o) => (Str.isString(o) ? Str.removeSMSDomain(o.value) : o.value) === currentValue && !o.isFake;

        // We use removeSMSDomain here in case currentValue is a phone number
        let defaultSelectedOption = _(this.options).find(removeSMSDomainIfPhoneNumber);
    ...
    getTruncatedOptions(currentValue, newAlreadySelectedOptions) {
        ...
        const dividerIndex = _.findIndex(this.options, findDivider);
        ...
        const formatOptions = (array) => _.chain(array).map(formatOption).sortBy(sortByOption).first(this.props.maxItemsToShow).value();

        const truncatedOptions = _.chain(splitOptions).map(formatOptions).flatten().value();
    ...
    setValue(val) {
        ...
        const deSelectOptions = (options) => _(options).map(deselectLogic);
    ...
    resetFocusedElements() {
        ...
        const setValueState = (state) => ({
            options: _(state.options).map(resetFocusedProperty),
        });
    ...
    performSearch(e) {
        ...
        const options = _(matches).map(filterOption);
        ...
        return (
            ...
            >
                <input
                    ...
                    value={this.props.propertyToDisplay === 'value' ? _.unescape(this.state.currentValue) : _.unescape(this.state.currentText.replace(/&nbsp;/g, ''))}
