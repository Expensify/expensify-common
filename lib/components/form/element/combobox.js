...
module.exports = React.createClass({
    propTypes: {
        ...
        // A property to allow the comboboxe set to any manual entry the user chooses
        propertyAllowAnyValue: React.PropTypes.bool
    },
    ...
    handleKeyDown(e) {
        ...
        case 13:
            ...
            let currentValue = this.state.options[this.state.focusedIndex].value;
            let currentText = this.state.options[this.state.focusedIndex].text;
            const newOptions = _(this.options).each((o) => {
                const option = o;
                option.isSelected = option.value === currentValue;
            });

            // if propertyAllowAnyValue is true and currentValue is absent then set it manually to what the user has entered
            if(!currentValue && this.props.propertyAllowAnyValue) {
