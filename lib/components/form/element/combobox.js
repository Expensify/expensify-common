...
module.exports = React.createClass({
    propTypes: {
        ...
        allowAnyValue: React.PropTypes.bool
    },
    ...
    getInitialState(noDefaultValue, newOptions) {
        ...
        const truncatedOptions = _(this.options).first(this.props.maxItemsToShow).map(option => ({
            ...
            focused: false,
            isSelected: this.originalValue
                ? option.value.toString() === this.props.originalValue
        ...
        const defaultSelectedOption = _(this.options).findWhere({value: this.originalValue || this.props.defaultValue});

        return {
            isDropdownOpen: this.props.openOnInit,
            currentValue: noDefaultValue ? this.originalValue || '' : this.props.defaultValue || '',
    ...
    /**
     * Hard sets the current text to want we want
     *
    ...
    /**
     * Function to check if we're currently over a category and if it has the error class.
     * If the div does not exist or if class is error is not present return false, else true
     *
     * @return {Bool}
     */
    categoryHasError() {
        const currentDiv = $(this.refs.input).parents('li').find('#editTaxCategory');
        const hasErrorClass = currentDiv ? $(currentDiv).parents('li').find('input').hasClass('error') : false;
     ...
     */
    handleClickAway(e) {
        // Lets check if the class has an error before this.reset(true) is called
        const isCategoryError = this.categoryHasError();
        ...
        if (isCategoryError) {
            ...
            this.props.onChange(this.originalText || this.props.defaultValue);
            this.setError();
        } else {
            // Trigger a change so that inline editor knows to cancel itself
            this.props.onChange(this.originalValue || this.props.defaultValue);
        }
    },
    ...
    handleKeyDown(e) {
        ...
        case 13: {
            ...
            newOptions = _(this.options).each((o) => {
                const option = o;
                option.isSelected = option.value === currentValue;
            });
        ...
        case 9:
        case 27:
            // Set the value to what was previously chosen and then close the dropdown
            if (this.categoryHasError()) {
                this.props.onChange(this.originalText || this.props.defaultValue);
                this.setError();
            } else {
                this.props.onChange(this.originalValue || this.props.defaultValue);
    ...
    handleClick(selectedValue) {
        ...
        let currentText = '';
        const newOptions = _(this.options).each((o) => {
            const option = o;
