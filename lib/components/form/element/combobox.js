...
module.exports = window.CreateClass({
    propTypes: {
        ...
        // A list of options to show as selected too.
        alreadySelectedOptions: window.PropTypes.array,
        ...
        onlyShowWhileOpen: window.PropTypes.bool
    },
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (!_.isUndefined(nextProps.options) && !_.isEqual(nextProps.options, this.props.options)) {
            this.reset(false, nextProps.options);
        }
    },
    ...
    getTruncatedOptions(currentValue) {
        ...
        const truncatedOptions = _(this.options).first(this.props.maxItemsToShow).map(option => ({
            ...
            focused: false,
            isSelected: _.isEqual(option.value, currentValue) || Boolean(_.find(this.props.alreadySelectedOptions, other => other.value === option.value))
        }));
    ...
    setValue(val) {
        ...
        const newOptions = _(this.state.options).map((o) => {
            const option = o;
            option.isSelected = _.isEqual(option.value, val) || Boolean(_.find(this.props.alreadySelectedOptions, other => other.value === option.value));
    ...
    performSearch(e) {
        ...
        const options = _(matches).map(option => ({
            ...
            disabled: option.disabled,
            focused: false,
            isSelected: _.isEqual(option.value, e.target.value) || Boolean(_.find(this.props.alreadySelectedOptions, other => other.value === option.value))
        }));
    ...
    render() {
        if (this.props.onlyShowWhileOpen && !this.state.isDropdownOpen) {
            return null;
        }
            ...
            >
                ...
                <div className={React.classNames(inputGroupBtnClasses)}>
                    ...
                    {this.state.isDropdownOpen && <React.c.Dropdown
                        options={this.state.options}
                        alreadySelectedOptions={this.props.alreadySelectedOptions}
