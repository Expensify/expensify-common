...
class DropDownItem extends React.Component {
     ...
     */
    createTooltip = () => {
        if (!this.state.hasInstantiatedTooltip) {
            $(`.expensify-dropdown [data-value="${this.props.value}"] [data-toggle="tooltip"]`).tooltip({
            ...
            this.setState({
                hasInstantiatedTooltip: true
            });
        }
    }

    /**
     * Unselectively destroy any combobox generated tooltip in case there are any stale ones left
     */
    destroyTooltip = () => {
        if (this.state.hasInstantiatedTooltip) {
