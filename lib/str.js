...
module.exports = {
    ...
    /**
     * Checks that the string is a list of comma separated email addresss.
     *
     ...
     *
     * @param {String} str The string to check for login validity.
     *
     * @returns {Boolean} True if all entries are valid or if input is empty
     */
    areValidLogins(str) {
        let string = module.exports.removeTrailingComma(str);
        if (string === '') {
            return true;
        }

        return string.split(',').every(function(login) {
            login = login.trim();
            return module.exports.isValidPhone(login) || module.exports.isValidEmail(login);
        });
    },
     ...
     */
    isValidPhone(phone) {
        return CONST.REG_EXP.SMS_E164_PLUS.test(phone);
    },
    ...
    removeSMSDomain(text) {
        return text.replace(CONST.SMS_DOMAIN, '');
    },
     ...
     */
    isSMSLogin(text) {
        return module.exports.isValidPhone(module.exports.removeSMSDomain(text));
    },
};
