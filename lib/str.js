...
module.exports = {
    ...
    truncateInMiddle(fullStr, maxLength) {
        ...
        halfLengthToShow = (maxLength - separator.length) / 2;
        beginning = fullStr.substr(0, Math.ceil(halfLengthToShow));
        end = fullStr.substr(fullStr.length - Math.floor(halfLengthToShow));
     ...
     *
     * @param {String} string The string to check for emails validity.
     *
     * @returns {Boolean} True if all emails are valid or if input is empty
     */
    areValidEmails(string) {
        string = module.exports.removeTrailingComma(string);
        if (string === '') {
            return true;
        }
    ...
    sanitizePhoneNumber(string) {
        ...
        return string.length >= 10 ? string : null;
    },
