...
module.exports = {
     ...
     * @param {String} character You can input more than one character, but it will only return the size of the first one.
     * @returns {Number} Byte size of the character
     */
    getRawByteSize(character) {
    ...
    getByteLength(input) {
        ...
        for (var i = 0; i < input.length; i++) {
            byteLength += this.getRawByteSize(input[i]);
        }
        return byteLength;
    },
    ...
    shortenByByte(input, maxSize) {
        ...
        for (var i = 0; i < input.length; i++) {
            var charByteSize = this.getRawByteSize(input[i]);
            if ((charByteSize + totalByteLength) > maxSize) {
                return input.substr(0, i - 3) + '...';
            } else {
