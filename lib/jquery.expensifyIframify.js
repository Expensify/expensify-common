...
export default {
    ...
    load($) {
        ...
        function postMessage(name, data, postToIframe) {
            ...
            if (!postToIframe && window.parent !== window) {
                try {
                    ...
                    // Fall back to inferring the parent origin from allowedCommunications.
                    const allowedTargets = allowedCommunications[settings.origin];
                    if (allowedTargets) {
                        for (let i = 0; i < allowedTargets.length; i += 1) {
                            if (allowedTargets[i] !== settings.origin) {
                                targetOrigin = allowedTargets[i];
                                break;
                            }
                        }
                    }
            ...
            if (postToIframe && iframeElement[0].contentWindow) {
                try {
                    ...
                    // Fall back to extracting the origin from the iframe's src attribute.
                    try {
                        const src = iframeElement.attr('src') || iframeElement[0].src;
                        if (src) {
                            const url = new URL(src, window.location.href);
                            targetOrigin = `${url.protocol}//${url.hostname}`;
                        }
                    } catch (urlError) {
                        log('could not determine iframe origin', urlError);
                    }
                }
                if (targetOrigin) {
        ...
        function handleWindowMessage(e) {
            ...
            if (
                allowedCommunications[event.origin] &&
