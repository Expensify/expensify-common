...
module.exports = {
    ...
    load($, _) {
        ...
        /**
         * This indicates communication from which subdomains to which subdomains is allowed.
         * Keys are from and values are to. Eg: www can talk to secure and salesforce.
         * @type {Object}
         */
        const whiteListedSubdomains = {
            www: ['secure', 'salesforce'],
            secure: ['www'],
            staging: ['staging-secure'],
            staging2: ['staging-secure'],
            'staging-secure': ['staging'],
            salesforce: ['secure']
        };
        ...
        function postMessage(name, data, postToIframe) {
            ...
            if (postToIframe && self[0].contentWindow) {
                ...
                // Handle production subdomains
                if (nonsecureSubdomain !== 'staging' && nonsecureSubdomain !== 'staging2' && settings.origin.search(`//${nonsecureSubdomain}.expensify`) > -1) {
                    ...
                    newOrigin = settings.origin.replace(/\/\/staging\d*\.expensify/, '//staging-secure.expensify');
                }
                log('posting message to', newOrigin, msg);
        ...
        function handleWindowMessage(event) {
            ...
            const incomingDomainArray = event.source.location.hostname.split('.');
            const incomingSubdomain = incomingDomainArray.shift();
            const incomingDomainWithoutSubdomain = incomingDomainArray.join('.');
            if (incomingDomainWithoutSubdomain !== 'expensify.com' && incomingDomainWithoutSubdomain !== 'expensify.com.dev') {
                return;
            }
            const domainArray = event.target.location.hostname.split('.');
            const subdomain = domainArray.shift();
            const domainWithoutSubdomain = domainArray.join('.');
            if (domainWithoutSubdomain !== 'expensify.com' && domainWithoutSubdomain !== 'expensify.com.dev') {
                return;
            }

            const isAllowedRequest = _(whiteListedSubdomains[incomingSubdomain]).some(function (to) {
                return to === subdomain;
            });
            ...
            let nameParts = msgParts[0].split(':');
