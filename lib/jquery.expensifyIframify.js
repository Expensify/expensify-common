...
module.exports = {
    ...
    load($) {
         ...
         */
        var _defaultOrigin = window.location.protocol + "//" + window.location.hostname;

        /**
         * Any events coming from these subdomains are OK
         * @type {String[]}
         */
        var _whiteListedSubdomains = ['www', 'secure', 'staging-secure', 'staging'];
            ...
            off: function (name) {
                if (self.iframeId) {
                    ...
                    if (!name) {
                        ...
                        $.each(eventHandlers, function (i, obj) {
                            delete obj[name];
                        });
                    }
                }
        ...
        function handleWindowMessage(event) {
            ...
            urlArray = event.origin.split('://');
            if (urlArray && urlArray.length && urlArray.length > 1) {
                domainArray = urlArray[1].split('.');
                originSubdomain = domainArray.shift();
                domainWithoutSubdomain = domainArray.join('.');

                allowedOrigin = _(_whiteListedSubdomains).find(function (subdomain) {
                    return event.origin === urlArray[0] + '://' + subdomain + '.' + domainWithoutSubdomain;
                });
        ...
        function postMessage(name, data, postToIframe) {
            ...
            if (!postToIframe && window.parent !== window) {
                ...
                if (settings.origin.search('//secure.expensify') > -1) {
                    newOrigin = settings.origin.replace('//secure.expensify', '//www.expensify');
            ...
            if (postToIframe && self[0].contentWindow) {
                ...
                if (settings.origin.search('//www.expensify') > -1) {
                    newOrigin = settings.origin.replace('//www.expensify', '//secure.expensify');
        ...
        $.fn.expensifyIframeify = function (actionOrOptions, args, callback) {
            const urlArray = _defaultOrigin.split('://');
            const domainArray = urlArray[1].split('.');
            const originSubdomain = domainArray.shift();
            ...
            self.iframeId = window.top === window
