...
/**
 * Decides whether the optimized candidate scanner can run without changing HTML-aware matching.
 *
 * When text is escaped, user-provided angle brackets cannot become active HTML. When raw HTML is
 * allowed, however, the original regexes can use a distant `<...>` or `>` boundary while deciding
 * whether a URL or Markdown pair is valid. We therefore keep the full-text path for raw-HTML input
 * that contains angle brackets.
 *
 * @param text - The text currently being processed, including markup generated by earlier rules.
 * @param shouldEscapeText - Whether user-provided HTML characters are escaped before processing.
 * @returns Whether candidate scanning can safely be used for this text.
 */
